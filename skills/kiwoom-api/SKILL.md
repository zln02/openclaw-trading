...
OpenClaw 제이(J)가 텔레그램에서 아래 패턴을 인식하면, 반드시 `exec` 도구로 해당 파이썬 스크립트를 실행한 뒤 결과를 docs/architecture/SOUL.md에 정의된 투자/데이터 답변 스타일(마크다운 표 + 다음 단계 권장사항)로 요약해서 답변하게 해.
...
- **"현재가 [종목명]" 또는 "SK하이닉스"**
  ...
  - 동작: exec 도구로 아래 명령 실행
    ...
    ```bash
    cd /home/wlsdud5035/.openclaw/workspace/skills/kiwoom-api && \
