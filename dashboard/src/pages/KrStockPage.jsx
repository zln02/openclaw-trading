...
function RetBadge({ value, label }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-0.5 text-xs ${up ? "profit-text" : "loss-text"}`}
    >
          ...
          {kiwoomError || !kiwoom ? (
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <div
                  className={`font-mono font-medium ${(kiwoom.cumulative_pnl ?? 0) >= 0 ? "profit-text" : "loss-text"}`}
                >
                  {fmtKrw(kiwoom.cumulative_pnl)}
                  {kiwoom.cumulative_pnl_pct != null && (
                    <span className="text-xs ml-1 opacity-75">
                      ({pct(kiwoom.cumulative_pnl_pct)})
                    </span>
                  )}
                </div>
              </div>
              <div>
                ...
                <div
                  className={`font-mono font-medium ${(kiwoom.today_pnl ?? 0) >= 0 ? "profit-text" : "loss-text"}`}
                >
            ...
            {(topStocks ?? []).length > 0 ? (
              topStocks.slice(0, 8).map((stock) => (
                <div
                  ...
                  className="flex items-center justify-between text-sm px-1 py-0.5 rounded hover:bg-card/30"
                >
                  <div className="flex items-center gap-2">
                    <span className="font-mono text-xs text-text-secondary w-14">
                      {stock.stock_code}
                    </span>
                    <span className="font-medium text-text-primary truncate max-w-[100px]">
                      {stock.stock_name}
                    </span>
                  </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border">
                  {["종목", "수량", "평균단가", "현재가", "평가금액", "손익금액", "수익률"].map(
                    (h) => (
                      <th
                        key={h}
              ...
              <tbody>
                {positions.map((pos) => (
                  ...
                  >
                    ...
                    <td
                      className={`text-right py-3 px-3 font-mono ${(pos.pnl_amount ?? 0) >= 0 ? "profit-text" : "loss-text"}`}
                    >
                      {fmtKrw(pos.pnl_amount)}
                    </td>
                    <td
                      className={`text-right py-3 px-3 font-mono font-semibold ${(pos.pnl_pct ?? 0) >= 0 ? "profit-text" : "loss-text"}`}
                    >
                      {pct(pos.pnl_pct)}
                    </td>
                  </tr>
