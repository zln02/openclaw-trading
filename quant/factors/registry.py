...
class FactorContext:
    ...
    def _load_series_from_supabase(self, symbol: str, market: str) -> List[dict]:
        ...
        try:
            ...
            log.warning("supabase series fetch failed", symbol=symbol, market=market, error=str(exc))
            return []
    ...
    def _load_series_from_yfinance(self, symbol: str, market: str) -> List[dict]:
        try:
            ...
            log.warning("yfinance series fetch failed", symbol=symbol, market=market, error=str(exc))
            return []
    ...
    def get_fundamentals(self, symbol: str, market: str = "kr") -> dict:
        ...
        if mk == "kr" and self.supabase:
            ...
            try:
                ...
                log.warning("KR fundamentals fetch failed", symbol=symbol, error=str(exc))
        ...
        if not data:
            try:
                ...
                log.warning("yfinance fundamentals fetch failed", symbol=symbol, market=market, error=str(exc))
                data = {}
    ...
    def get_orderbook_imbalance(self, symbol: str, market: str = "kr") -> float:
        ...
        try:
            ...
            log.warning("orderbook imbalance fetch failed", symbol=symbol, market=market, error=str(exc))
...
def calc(
    ...
    try:
        ...
        log.warning("factor calc failed", factor=factor_name, symbol=symbol, as_of=as_of_iso, error=str(exc))
        return 0.0
